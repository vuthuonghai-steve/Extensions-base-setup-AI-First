...
def _latest_log_file(tmp_path: Path) -> Path:
    logs_dir = tmp_path / ".agents" / "hooks" / "logs"
...
def test_log_never_raises_when_logs_dir_unwritable(tmp_path, monkeypatch, capsys):
    # .agents là file thường -> không thể tạo thư mục con -> log phải im lặng bỏ qua.
    broken_root = tmp_path / "broken"
    broken_root.mkdir()
    (broken_root / ".agents").write_text("i am a file, not a dir")
