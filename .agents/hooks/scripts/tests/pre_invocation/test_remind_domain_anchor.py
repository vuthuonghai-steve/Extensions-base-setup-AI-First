...
def read_log_lines(tmp_path: Path) -> list[dict]:
    """Đọc mọi dòng JSONL trong <tmp>/.agents/hooks/logs/ (repo giả)."""
    log_dir = tmp_path / ".agents" / "hooks" / "logs"
    lines: list[dict] = []
...
def test_scan_skips_node_modules_and_git(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Anchor trong node_modules/.git/.agents phải bị bỏ qua -> fail-open {}."""
    for skipped_dir in ("node_modules", ".git", ".agents"):
